...
class MechanicalTurk(object):
	def __init__(self,mturk_config_loc=None):
		...
		if mturk_config_loc is None:
			self.mturk_config_dict = json.load(open("mturkconfig.json"))
		else:
			self.mturk_config_dict = json.load(open(mturk_config_loc))
		if not self.mturk_config_dict.get("stdout_log"):
			logging.getLogger('requests').setLevel(logging.WARNING)

		self.sandbox = self.mturk_config_dict.get("use_sandbox") == True # Use sandbox?
		self.verify_mturk_ssl = self.mturk_config_dict.get("verify_mturk_ssl") == True
		self.aws_key = self.mturk_config_dict["aws_key"]
		self.aws_secret_key = str(self.mturk_config_dict["aws_secret_key"])
